...
namespace MalbersAnimations
{
    public class JumpNoRootBehaviour : StateMachineBehaviour
    {
        ...
        override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
        {
            ...
            Vector3 JumpVector = (Vector3.up * JumpMultiplier * animal.JumpHeightMultiplier) + (animator.transform.forward * ForwardMultiplier * animal.JumpForwardMultiplier);
        ...
        override public void OnStateExit(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
        {
            ...
            if (rb && currentState.tagHash == Hash.Fly)
            {
                Vector3 cleanY = new Vector3(rb.velocity.x, 0, rb.velocity.z);
                rb.velocity = cleanY;
            }

            if (currentState.tagHash != Hash.Fall && currentState.tagHash != Hash.Fly)
            {
